...
```{r prepare_ipath3}
func_sign_over <- da.otus_table %>% 
  filter(log2FoldChange > 0 ) %>%
  select(OTU, log2FoldChange, padj)

func_sign_under <- da.otus_table %>% 
  filter(log2FoldChange < 0 ) %>%
  select(OTU, log2FoldChange, padj)
...
func_sign_over <- func_sign_over %>% select(OTU, Color, Width)
func_sign_under <- func_sign_under %>% select(OTU, Color, Width)
