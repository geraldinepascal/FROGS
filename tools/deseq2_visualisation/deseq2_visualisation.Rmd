...
```{r prepare_ipath3}
print(da.otus_table)
da.functions_table <- rename(da.otus_table, FUNC = OTU)
func_sign_over <- da.functions_table  %>% 
  filter(log2FoldChange > 0 ) %>%
  select(FUNC, log2FoldChange, padj)

func_sign_under <- da.functions_table  %>% 
  filter(log2FoldChange < 0 ) %>%
  select(FUNC, log2FoldChange, padj)

func_sign_over <- func_sign_over %>% 
  mutate(Color = if_else(log2FoldChange < 3, 1,
   if_else(log2FoldChange < 6, 2,
   if_else(log2FoldChange < 9, 3,
   if_else(log2FoldChange < 12, 4, 5))))) %>%
  mutate(Width = if_else(padj < 0.00001, "10", 
   if_else(padj < 0.0001, "8",
   if_else(padj < 0.001, "6",
   if_else(padj < 0.01, "4", "2")))))
