...
<div style="display:flex; align-items:center; gap:15px; margin-bottom:20px;">
  ...
  <h1 style="margin:0;">Stat: DESeq2 Visualisation<i><span class="text-accent">(`r params$tool` v`r params$version`)</span></i></h1>
</div>
...
## Differentially abundant `r params$analysis` table
...
feature <- ifelse(params$analysis=="ASV","ASVs","Functions")
doc <- ""
...
# qualitative variable
     ...
     doc <- paste0("You chose to compare ", params$mod1, " with ", params$mod2," reference. This implies that a positive log2FoldChange means more abundant in ", params$mod1, " than in ", params$mod2, ".")
     cat(doc)
}
...
Significant `r feature` (adjusted p-value using Benjamini Hochberg correction < `r params$padj_th`) were extracted and sorted by adjusted p-value, and enriched with `r if(params$analysis=="ASV"){"taxonomic"}else{"functional"}` classification.
...
### Visualisation of differentially abundant functions with iPath 3.
```
```{r doc_path, eval=run_ipath, echo=run_ipath, message=FALSE}
# Import input files
if(params$mod1 != "None") {
     doc <- paste0("You chose to compare ", params$mod1, " with ", params$mod2, " reference. This implies that the over-abundants pathways (first image) and under-abundant pathways (second image) are involved in ", params$mod1, " condition.")
     cat(doc)
} else {
     doc <- "The first image (green paths) represents the over-abundants involved pathways. The second image (red paths) represents the under-abundant pathways."
     cat(doc)
}
```
...
Use the two files **ipath_under.tsv** and **ipath_over.tsv** as input to visualise and explore metabolic pathways with the [iPath3 website](https://pathways.embl.de/).
```
...
Pie <- ggplot(da_pie, aes(x="", y=Number, fill=IDs)) + 
           ...
           labs(x = NULL, y = NULL, fill = NULL, title = paste0("Pie chart to view ",feature," number of Differential Abundance test")) +
           theme_classic() + theme(
...
da_volcano <- da_volcano %>%
    mutate(
    	color = case_when(
    		lfc > 0 & evidence > y_axix_volcano_line ~ "More abundant", 
    		lfc < 0 & evidence > y_axix_volcano_line ~ "Less abundant", 
...
volcano_plot <- ggplot(da_volcano, aes(x = lfc, y = evidence)) + 
    ...
    theme_bw(base_size = 16) + theme(legend.title = element_blank()) +           # clean up theme and remove legend title
    ggtitle(label = "Volcano Plot", subtitle = paste0(feature," colored by effect sign")) +        # add title
    ...
    scale_color_manual(values = c("More abundant" = "#C6792B", "Less abundant" = "#759493", "Equal abundance" = "#C0C0C0")) # change colors
...
cat(paste0("There is not engough (0 or 1) differentially abundant ",feature," to produce heatmap plot."))
```
...
if ( length(result_names)==2 || params$mod1 != "None"){
	...
	hplot <- plot_heatmap(hplot_input, 
				...
				ggtitle(paste0("Heatmap plot of DA ",feature,", between 2 conditions"), subtitle = subtitle)
## Quantitative variable
} else {
    ...
    hplot <- plot_heatmap(hplot_input, 
             ...
             scale_y_discrete(feature) +
             ggtitle(paste0("Heatmap plot of DA ",feature," with samples ordered according to ", params$var))
}
# ggplotly(hplot)
hplot
```
