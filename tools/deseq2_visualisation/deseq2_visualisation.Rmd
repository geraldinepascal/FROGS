...
params:
  ...
  analysis:
    value: x
  # Over represented functions svg file
  over_svg:
    value: x
  # Under represented functions svg file
  under_svg:
    value: x
---
...
over_svg <- params$over_svg
under_svg <-  params$under_svg
```
...
```
```{r over_func, echo=FALSE, out.width = '100%', eval = run_ipath}
img_data <- readBin(over_svg, "raw", file.size(over_svg))
img_base64 <- RCurl::base64(img_data)
cat(paste0("<img src='data:image/jpg;base64,",img_base64,"'/>"))
```
...
```{r under_func, echo=FALSE, out.width = '100%',eval = run_ipath}
img_data <- readBin(under_svg, "raw", file.size(under_svg))
