...
```{r prepare_ipath3}
over_otu <- da.otus_table %>% 
  filter(log2FoldChange > 0 ) %>%
  select(OTU, log2FoldChange, padj)
  
under_otu <- da.otus_table %>% 
  filter(log2FoldChange < 0 ) %>%
  select(OTU, log2FoldChange, padj)

write.table(over_otu, file="over.tsv", sep="\t", quote=FALSE, row.names = FALSE)
write.table(under_otu, file="under.tsv", sep="\t", quote=FALSE, row.names = FALSE)
...
```
![alt text](map.svg, toolTipMode=1) 
