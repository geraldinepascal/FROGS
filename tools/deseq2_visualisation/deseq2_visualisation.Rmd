...
Then we extract significant OTUs or FUNCs at the p-value adjusted threshold (after Benjamini Hochberg correction) and enrich results with taxonomic/functional classification and sort the results by pvalue.
...
func_sign_under <- func_sign_under %>% 
  mutate(Color = if_else(log2FoldChange > -1, "#9B6F51",
   if_else(log2FoldChange > -2, "#DE9F73", "#B27F5C")))%>%
  mutate(Width = if_else(padj < 0.001, "W12",
   if_else(padj < 0.01, "W10", "W8")))
...
```{asis, eval=run_ipath, echo=run_ipath}
The darker the path, the higher the log2 fold change (<1, <2, or >2).
...
[WARNING] : The images (ipath_over.svg, ipath_under.svg) must remain in the same folder as this html report to continue to be displayed
```
...
Pie <- ggplot(da_pie, aes(x="", y=Number, fill=IDs)) + 
           ...
           labs(x = NULL, y = NULL, fill = NULL, title = "Pie chart to view OTUsor FUNCs number of Differential Abundance test") +
           theme_classic() + theme(
...
cat("There are no differentially abundant OTUsor FUNCs. Please use a higher threshold for the adjusted p-value or consider a different comparison.")
```
...
if ( length(result_names)==2 || params$mod1 != "None"){
	...
	hplot <- plot_heatmap(prune_taxa(da.abundances_table$id, abundances_input), 
				...
				ggtitle("Heatmap plot of DA otus or functions, between 2 conditions", subtitle = subtitle)
## Quantitative variable
} else {
    ...
    hplot <- plot_heatmap(prune_taxa(da.abundances_table$id, abundances_input), 
             ...
             ggtitle(paste0("Heatmap plot of DA otus or functions with samples ordered according to ", params$var))
}
# ggplotly(hplot)
hplot
```
