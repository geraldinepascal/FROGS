...
if __name__ == "__main__":
    ...
    parser.add_argument('--java-mem', type=int, default=2, help="Java memory allocation in Go. [Default: %(default)s]")
    parser.add_argument('--taxonomy-ranks', nargs='*', default=["Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species"], help='The ordered ranks levels present in the reference databank. [Default: %(default)s]' )
    parser.add_argument('--rdp', default=False,  action='store_true',  help="Use RDP classifier to affiliate ASV [Default: %(default)s]")
    ...
    group_input.add_argument('--reference', required=True, help='Preformated reference file (format: blast-indexed FASTA).')
    group_input.add_argument('--input-biom', required=True, help='BIOM file (format: BIOM).')
    group_input.add_argument('--input-fasta', required=True, help="FASTA file of ASV's seed (format: FASTA).")
    # Outputs
