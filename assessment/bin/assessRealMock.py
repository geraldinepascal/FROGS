    ...
    if cleaned_taxonomy[0].startswith("d:"):
        ...
        for taxa in cleaned_taxonomy:
            while not taxa.startswith(ranks[rank_idx]) and taxa != "Multi-affiliation" and taxa != "unclassified":
                ...
                rank_idx += 1
            if taxa != "Multi-affiliation" and taxa != "unclassified":
                taxa = taxa[2:]
            tmp_tax.append(taxa)
            rank_idx += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument( '-r', '--checked-ranks', nargs='*', default=["Domain", "Phylum", "Class", "Order", "Family", "Genus"], help='Evaluated ranks.' )
