...
if __name__ == "__main__":
    ...
    if args.affiliation_databank_fasta is None or args.affiliation_databank_tax is None:
        ...
        if not os.path.exists("stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list."+ str(args.otu_distance) + ".cons.taxonomy"):
			exec_cmd( 'ln -sf stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.unique.cons.taxonomy' + ' stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.'+ str(args.otu_distance) + '.cons.taxonomy' )
        ...
        if not os.path.exists("stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.0.03.biom"):
			exec_cmd( 'ln -sf stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.unique.biom ' + args.output_biom )
        else:
        ...
        ##gp## mise en commentaire pour assessment real
        # Degap and rename OTU
        ##gp##exec_cmd( 'mothurDeGapSeeds.py ' \
            ##gp##+ '--input stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.' + str(args.otu_distance) + '.rep.fasta ' \
            ##gp##+ '--output stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list.' + str(args.otu_distance) + '.rep.degap.fasta' )

        # Add reference ID in seeds descriptions
        ##gp##exec_cmd( 'mothurAddSeedRef.py ' \
            ##gp##+ "--input stability.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.an.unique_list." + str(args.otu_distance) + ".rep.degap.fasta "  \
            ##gp##+ "--reads " + " ".join(reads) + " " \
            ##gp##+ "--trimmed-reads stability.trim.contigs.good.unique.good.filter.fasta " \
            ##gp##+ "--output " + args.output_fasta )
